// Taken from https://github.com/Loopring/protocols
...
function addPoint(a, b) {
  ...
  res[1] = F.div(F.add(delta, F.sub(F.mul(exports.A, beta), gamma)), F.sub(F.one, dtau))
...
function inCurve(P) {
  ...
  if (!F.eq(F.add(F.mul(exports.A, x2), y2), F.add(F.one, F.mul(F.mul(x2, y2), exports.D)))) {
    return false
  }
